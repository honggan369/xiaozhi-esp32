...
class Lcs3OttoRobotBoard : public WifiBoard {
private:
    ...
    void EnsureOttoControllerInitialized() {
        ...
        InitializeOttoPca9685(i2c_bus_, SERVO_PCA9685_I2C_ADDR, SERVO_PCA9685_FREQ_HZ);
        InitializeOttoController(LCS3_OTTO_CONFIG);
        otto_controller_initialized_ = true;
        ESP_LOGI(TAG, "Otto controller initialized with PCA9685");
    }

public:
    Lcs3OttoRobotBoard() : boot_button_(BOOT_BUTTON_GPIO) {
        InitializeI2c();
        InitializeSpi();
...
DECLARE_BOARD(Lcs3OttoRobotBoard);
